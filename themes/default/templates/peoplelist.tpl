		...
		<tr>
			...
			{foreach from=$person item=p name=person}   
				...
				<td class="cover" style="width: {$hundred/$pmp_people_per_row}%; vertical-align: top;">
					<a href="index.php?content=searchperson&amp;name={$p.Name|rawurlencode}&amp;nowildcards{if isset($p.Birthyear)}&amp;pbirthyear={$p.Birthyear}{/if}">{$p.Name} {if isset($p.Birthyear)} ({$p.Birthyear}){/if}<br /><br />
